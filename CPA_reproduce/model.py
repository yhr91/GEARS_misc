...
class ComPert(torch.nn.Module):
    ...
    def __init__(
        ...
        self.pert2neighbor = None
        self.pert_dict_rev = None
    ...
    def compute_drug_embeddings_(self, drugs):
        ...
        if self.emb_kg == 'kg':
            ### use neighborhood set aggregation
            temp = self.dosers(drugs) @ self.drug_embeddings.weight
            ngbr_emb = []
            
            for i in drugs:
                out = torch.where(i == 1)[0].detach().cpu().numpy()                
                #if len(out) != 1:
                    ## not ctrl
                #    out = out[:-1]
                pert_agg = []
                for x in out:
                    pert_agg.append(self.pert2neighbor[self.pert_dict_rev[str(x)]])
                ngbr_emb.append(sum(pert_agg))
            
            ngbr_emb = torch.tensor(np.vstack(ngbr_emb)).float().to(self.device)
            kg_emb = self.emb_kg_encoder(ngbr_emb)            
            return kg_emb + temp
